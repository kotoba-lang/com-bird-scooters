(ns bird-scooters.main
  "Kotodama WASM entrypoint for the Bird_scooters clean-room actor (L4) — Clojure port.
  ...
  facts (`bird_scooters.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bird_scooters")
(def tier "L4")
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "birdscoo_dri"
   ...
   {:entity "Rating"   :plural "ratings"   :id-prefix "birdscoo_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"bird_scooters.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bird_scooters-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bird_scooters-compat/ready)
